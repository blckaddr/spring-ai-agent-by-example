...
import com.example.agent.capture.RecordingToolCallback;
import com.example.agent.capture.Step;
import com.example.agent.capture.StepCapture;
import com.example.agent.capture.StepCollector;
import com.example.agent.mcp.McpToolServerIndex;
...
/**
 * The single chat agent. Builds an Ollama {@link ChatClient}, wraps each of its <em>scoped</em> MCP
 * tools (currency + calculator — see {@code agent.chat.tool-servers}) in a
 * {@link RecordingToolCallback}, and runs the synchronous tool-calling loop. Spring AI drives the
 * loop internally (model decides → tool called → result fed back → repeat) until a final answer;
 * the wrapper makes each step visible.
 */
@Service
public class AgentService {
    ...
    /** Prefix for the fresh, one-off conversation id used when a caller supplies no sessionId. */
    private static final String ANON_SESSION_PREFIX = "anon-";
    ...
    public AgentResponse run(String request, String sessionId, java.util.function.Consumer<Step> stepListener) {
        // Memory is opt-in: a caller that wants it across turns passes a stable sessionId (Phase 3).
        // With no sessionId we mint a FRESH conversation id per call — each request is independent
        // (no shared/accumulating "default" memory), so a one-shot example is reproducible no matter
        // how many times, or in what order, it's run.
        String conversationId = (sessionId == null || sessionId.isBlank())
                ? ANON_SESSION_PREFIX + java.util.UUID.randomUUID()
                : sessionId;
